...
export function AdminDashboard() {
  ...
  const { stats, fetchVehicles } = useVehicleStore();
  const navigate = useNavigate();
  const [activeModal, setActiveModal] = useState<'vehicles' | 'password' | 'reports' | null>(null);

  useEffect(() => {
    console.log('🔧 AdminDashboard: Starting to fetch vehicles...');
    fetchVehicles().then(() => {
      console.log('✅ AdminDashboard: Vehicles fetched successfully');
    }).catch((error) => {
      console.error('❌ AdminDashboard: Error fetching vehicles:', error);
    });
  }, [fetchVehicles]);

  // Debug logs for stats changes
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-gray-600">Manage Vehicles</p>
              </div>
            </button>
      ...
      {activeModal === 'vehicles' && (
        <VehicleList onClose={() => setActiveModal(null)} />
      )}
      ...
      {activeModal === 'reports' && (
        <Reports onClose={() => setActiveModal(null)} />
      )}
    </div>
  );
}